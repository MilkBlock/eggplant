...
pub fn pat_vars(
    ...
    let expanded = match &input.data {
        Data::Struct(data_struct) => {
            ...
            let field_is_base_var = data_struct
                .fields
                .iter()
                .map(|f| {
                    matches!(
            ...
            let decode_member_calls = valued_tys
                ...
                .zip(field_is_base_var.iter())
                .map(|((ty, idx), is_base_var)| {
                    if *is_base_var {
                        quote!(#W::Value::new(values[plan.#idx]))
                    } else {
...
pub fn ensure_PR_contained(field: &mut Field) {
    // ensure PR is contained
    if let syn::Type::Path(type_path) = &mut field.ty {
        ...
        if let Some(last_segment) = type_path.path.segments.last_mut() {
            ...
            } else if let syn::PathArguments::AngleBracketed(args) = &mut last_segment.arguments {
                // check whether PR is contained
                let has_pr = args.args.iter().any(|arg| {
                    if let syn::GenericArgument::Type(type_arg) = arg {
