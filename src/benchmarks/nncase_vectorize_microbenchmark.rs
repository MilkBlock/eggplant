...
pub fn workload_spec() -> eggplant::helpers::report::BenchmarkWorkloadSpec<'static> {
    eggplant::helpers::report::BenchmarkWorkloadSpec {
        positioning: eggplant::helpers::report::nncase_benchmark_positioning("vectorize"),
        op_costs: OP_COSTS,
        target_root: "LogicalMatMul(LogicalExp(LogicalMatMul(q, k)), v)",
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    eggplant::helpers::bench_cli::parse_extract_bench_args(
        "nncase_vectorize_extract_bench",
        "Run the nncase vectorize microbenchmark with CLI progress bars",
        8,
        20,
        args,
    )
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    eggplant::helpers::bench_cli::parse_timeline_export_args(
        ...
        "Export nncase vectorize timeline data with CLI progress bars",
        8,
        12,
        "target/nncase_vectorize_timeline.json",
        args,
    )
}
...
fn seed_domain(seed_name: &'static str) {
    ...
    NncaseVectorizeTx::add_rule(
        ...
        |ctx, _pat| {
            let q = ctx.insert_vectorize_input("q".to_owned());
            let k = ctx.insert_vectorize_input("k".to_owned());
            let v = ctx.insert_vectorize_input("v".to_owned());
            let blocked = ctx.insert_blocked();
            let inner_mm = ctx.insert_logical_mat_mul(q, k);
            let exp = ctx.insert_logical_exp(inner_mm);
            ctx.insert_logical_mat_mul(exp, v);
            let unpacked_q = ctx.insert_unpack(q, blocked);
            ctx.insert_logical_exp(unpacked_q);
            let packed_k = ctx.insert_pack(k, blocked);
            ctx.insert_unpack(packed_k, blocked);
            let unpacked_v = ctx.insert_unpack(v, blocked);
            ctx.insert_pack(unpacked_v, blocked);
        },
    );
...
fn build_extract_target() -> impl EgglogNode + EgglogTy + 'static {
    let q = VectorizeInput::<NncaseVectorizeTx>::new("q".to_owned());
    q.commit();
    let k = VectorizeInput::<NncaseVectorizeTx>::new("k".to_owned());
    k.commit();
    let v = VectorizeInput::<NncaseVectorizeTx>::new("v".to_owned());
    v.commit();
