...
fn seed_domain(seed_name: &'static str) {
    ...
    NncaseTransposeTx::add_rule(
        ...
        |ctx, _pat| {
            let lhs = ctx.insert_input("lhs".to_owned());
            let rhs = ctx.insert_input("rhs".to_owned());
            let aux = ctx.insert_input("aux".to_owned());
    ...
    F: FnMut(ProgressEvent),
{
    ...
    NncaseTransposeTx::sgl().reset_for_bench();
...
fn build_extract_target() -> impl EgglogNode + EgglogTy + 'static {
    let lhs = Input::<NncaseTransposeTx>::new("lhs".to_owned());
    lhs.commit();
    let rhs = Input::<NncaseTransposeTx>::new("rhs".to_owned());
    rhs.commit();
