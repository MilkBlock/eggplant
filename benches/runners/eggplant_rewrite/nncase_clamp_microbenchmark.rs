...
enum ClampExpr {
    ...
    #[eggplant::typst("(({input} * {weight}))_f")]
    #[eggplant::precedence(60)]
    #[cost(1)]
...
fn register_rewrite_rules(rs: RuleSetId) {
    ...
    NncaseClampTx::add_rule(
        ...
        || {
            let inner = ClampExpr::query_leaf();
            let clamp = ClampZeroInf::query(&inner);
            #[eggplant::pat_vars]
            struct Pat {
                inner: ClampExpr,
                clamp: ClampZeroInf,
    ...
    F: FnMut(ProgressEvent),
{
    ...
    NncaseClampTx::sgl().reset_for_bench();
