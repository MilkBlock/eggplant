...
#[cfg(feature = "rustsat-extract")]
#[test]
fn timeline_cli_accepts_md_out() {
    let parsed = math_microbenchmark_no_calculus_timeline_export::parse_args([
        "timeline-export",
        "--md-out",
        "target/no_calculus_report.md",
    ]);
    assert_eq!(
        parsed.md_out.as_deref(),
        Some("target/no_calculus_report.md")
    );
}

#[cfg(feature = "rustsat-extract")]
#[test]
fn timeline_cli_accepts_plot_out() {
    let parsed = math_microbenchmark_no_calculus_timeline_export::parse_args([
        "timeline-export",
        "--plot-out",
        "target/no_calculus_plot.png",
    ]);
    assert_eq!(
        parsed.plot_out.as_deref(),
        Some("target/no_calculus_plot.png")
    );
}
