...
fn main() {
    eprintln!("nncase_vectorize_extract_bench requires --features rustsat-extract");
}
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rows = nncase_vectorize_microbenchmark::run_extract_comparison_with_options_and_progress(
        args.rewrite_iters + 1,
        args.max_rewrite_mem_gib,
        &args.extractors,
        args.max_extract_time_secs,
        |event| match event {
            nncase_vectorize_microbenchmark::ProgressEvent::RewriteIterationComplete {
                current,
                total,
                tuple_count,
                peak_memory_bytes,
            ...
            nncase_vectorize_microbenchmark::ProgressEvent::RewriteStoppedByMemoryCap {
                current,
                total,
                peak_memory_bytes,
                cap_bytes,
            ...
            nncase_vectorize_microbenchmark::ProgressEvent::ExtractPhaseStart {
                method,
                current,
                total,
            } => {
            ...
            nncase_vectorize_microbenchmark::ProgressEvent::ExtractPhaseComplete {
                method,
                current,
                total,
                elapsed_ms,
    ...
    print_extract_comparison_report(
        "Nncase Vectorize Extract Comparison",
