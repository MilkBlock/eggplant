...
fn ff_steps_core<S, L, Pre, Post, Nd: DisplayNode<Directed>, Ed: DisplayEdge<Directed, Nd>>(
    ui: &egui::Ui,
    g: &mut Graph<Nd, Ed>,
    target_steps: u32,
    ...
    Post: Fn(&mut S, Option<bool>),
{
    ...
    let mut state = GraphView::<Nd, Ed, S, L>::get_layout_state(ui);
    ...
    GraphView::<Nd, Ed, S, L>::set_layout_state(ui, new_state);
    done
}

#[allow(clippy::too_many_arguments)]
fn ff_until_stable_core<
    S,
    L,
    Metric,
    Pre,
...
pub type DefaultGraphView<'a> = GraphView<'a, DefaultNodeShape, DefaultEdgeShape>;
...
impl<S, L, Nd: DisplayNode<Directed>, Ed: DisplayEdge<Directed, Nd>> Widget
    for &mut GraphView<'_, Nd, Ed, S, L>
where
    S: LayoutState,
    L: Layout<S>,
{
...
impl<'a, S, L, Nd: DisplayNode<Directed>, Ed: DisplayEdge<Directed, Nd>> GraphView<'a, Nd, Ed, S, L>
where
    S: LayoutState,
    L: Layout<S>,
{
    ...
    pub fn new(g: &'a mut Graph<Nd, Ed>) -> Self {
        Self {
            g,
...
impl<S, L, Nd: DisplayNode<Directed>, Ed: DisplayEdge<Directed, Nd>> GraphView<'_, Nd, Ed, S, L>
where
    S: LayoutState,
    L: Layout<S>,
{
    ...
    pub fn reset(ui: &mut Ui) {
        GraphView::<Nd, Ed, S, L>::reset_metadata(ui);
        GraphView::<Nd, Ed, S, L>::reset_layout(ui);
    }
    ...
    pub fn fast_forward(ui: &egui::Ui, g: &mut Graph<Nd, Ed>, steps: u32)
    where
        S: LayoutState,
        L: Layout<S>,
    {
        ff_steps_core::<S, L, _, _, Nd, Ed>(ui, g, steps, None, |_s| None, |_s, _tok| {});
    }
    ...
    where
        ...
        L: Layout<S>,
    {
        ff_until_stable_core::<S, L, _, _, _, Nd, Ed>(
            ui,
            g,
            epsilon,
            max_steps,
    ...
    fn select_edge(&mut self, idx: EdgeIndex<IndexTy>) {
    ...
    fn set_drag_start(&mut self, idx: NodeIndex<IndexTy>) {
        let n = self.g.node_mut(idx).unwrap();
        n.set_dragged(true);
    ...
    fn set_drag_end(&mut self, idx: NodeIndex<IndexTy>) {
        let n = self.g.node_mut(idx).unwrap();
        n.set_dragged(false);
...
impl<S, L, Nd, Ed> GraphView<'_, Nd, Ed, S, L>
where
    ...
    L: Layout<S>,
{
    ...
    pub fn fast_forward_force_run(ui: &egui::Ui, g: &mut Graph<Nd, Ed>, steps: u32) {
        ff_steps_core::<S, L, _, _, Nd, Ed>(
            ui,
            g,
            steps,
